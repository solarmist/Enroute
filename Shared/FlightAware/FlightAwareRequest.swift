...
class FlightAwareRequest<Fetched> where Fetched: Codable, Fetched: Hashable {
    ...
    private func handleResults(_ newResults: Set<Fetched>, age: TimeInterval = 0, isCacheable: Bool = true) {
        ...
        let sequencing = (age == 0
                            && added == batchSize
                            && results.value.count < howMany
                            && fetchSequenceCount < (howMany - batchSize - 1) / batchSize
        )
        ...
        if interval > 0 { // }, urlRequest != nil {
            ...
            fetchTimer = Timer.scheduledTimer(withTimeInterval: interval, repeats: false, block: { [weak self] _ in
    ...
    static func authorizedURLRequest(
        query: String,
        credentials: String? = Bundle.main.object(forInfoDictionaryKey: "FlightAware Credentials") as? String
    ) -> URLRequest? {
        let flightAware = "https://flightxml.flightaware.com/json/FlightXML2/"
        if let url = URL(string: flightAware + query),
           let credentials = (credentials?.isEmpty ?? true) ? nil : credentials?.base64 {
...
extension Date {
    ...
    static var currentFlightTime: Date {
        ...
        if credentials == nil || credentials!.isEmpty,
           !flightSimulationData.isEmpty,
           let simulationDate = flightSimulationDate {
